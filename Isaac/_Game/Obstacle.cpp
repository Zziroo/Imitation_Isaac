...
void Obstacle::Init()
{
	...
	case ObstacleTypes::BONFIRE:							// 이미지의 프레임 X값 : 애니메이션 / 이미지의 프레임 Y값 : 데미지를 입고(충돌) 불이 꺼지는 과정
		...
		objectSize = 68.0f;
		SubstituteObstacleShape(pos.x, pos.y, objectSize);
	...
	case ObstacleTypes::ITEMSTAND:
		...
		objectSize = 82.0f;
		SubstituteObstacleShape(pos.x, pos.y, objectSize, 0.0f, 0.0f, 0.0f, -9.5f);
	...
	case ObstacleTypes::SLIDER:								// 이미지의 프레임 X값 : 공격 전환(충돌 시 데미지 줌)
		...
		SubstituteObstacleShape(pos.x, pos.y, objectSize, 5.0f, 5.0f, 0.0f, -5.0f);
		obstacleInfo.doDamage = true;
		break;
...
void Obstacle::SubstituteObstacleShape(float posX, float posY, float size, float adjustSizeLeft, float adjustSizeTop, float adjustSizeRight, float adjustSizeBottom)
{
